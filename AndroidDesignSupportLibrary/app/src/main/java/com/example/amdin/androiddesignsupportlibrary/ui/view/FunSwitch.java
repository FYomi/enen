 ...
 *
 * @Override public void onSwitchChanged(boolean isCheck) {
 * if (isCheck) {
 * Toast.makeText(getActivity(), "yohoo", Toast.LENGTH_SHORT).show();
 * } else {
 * Toast.makeText(getActivity(), "aha", Toast.LENGTH_SHORT).show();
...
public class FunSwitch extends View implements ValueAnimator.AnimatorUpdateListener, ValueAnimator.AnimatorListener {
    ...
    public FunSwitch(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.FunSwitch);
        mIsOpen = ta.getBoolean(R.styleable.FunSwitch_checked1, false);
        ta.recycle();
        init(context);
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mListener != null) {
                    mListener.onSwitchChanged(mIsOpen);
                }
                invalidate();
                return true;
        }
